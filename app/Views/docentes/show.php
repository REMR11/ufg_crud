<?php echo $this->extend('layouts/main');?>
<?php echo $this->section('content');?>

<div class="container mt-5">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card shadow">
                <div class="card-header bg-info text-white">
                    <h4 class="mb-0">
                        <i class="bi bi-person"></i> Detalles del Docente
                    </h4>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <h6 class="text-muted">
                            <i class="bi bi-person-badge"></i> Nombre
                        </h6>
                        <p class="h5"><?php echo esc($docente['nombre']);?></p>
                    </div>

                    <div class="mb-3">
                        <h6 class="text-muted">
                            <i class="bi bi-person-badge"></i> Apellido
                        </h6>
                        <p class="h5"><?php echo esc($docente['apellido']);?></p>
                    </div>

                    <div class="mb-3">
                        <h6 class="text-muted">
                            <i class="bi bi-envelope"></i> Email
                        </h6>
                        <p><strong><?php echo esc($docente['email']);?></strong></p>
                    </div>

                    <div class="mb-3">
                        <h6 class="text-muted">
                            <i class="bi bi-telephone"></i> Teléfono
                        </h6>
                        <p><strong><?php echo esc($docente['telefono'] ?? 'N/A');?></strong></p>
                    </div>

                    <hr>

                    <div class="d-flex gap-2 flex-wrap">
                        <a href="<?php echo base_url('docentes/' . $docente['id'] . '/materias');?>" class="btn btn-secondary">
                            <i class="bi bi-journal-bookmark"></i> Ver Materias
                        </a>
                        <a href="<?php echo base_url('docentes/edit/' . $docente['id']);?>" class="btn btn-warning">
                            <i class="bi bi-pencil"></i> Editar
                        </a>
                        <a href="<?php echo base_url('docentes/delete/' . $docente['id']);?>" class="btn btn-danger" 
                           onclick="return confirm('¿Estás seguro de que deseas eliminar este docente?');">
                            <i class="bi bi-trash"></i> Eliminar
                        </a>
                        <a href="<?php echo base_url('docentes');?>" class="btn btn-secondary">
                            <i class="bi bi-arrow-left"></i> Volver
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<?php echo $this->endSection();?>
