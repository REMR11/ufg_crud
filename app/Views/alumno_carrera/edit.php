        ...
        <div class="col-md-8">
            <div class="card shadow">
                <div class="card-header bg-warning text-dark">
                    <h3 class="mb-0"><i class="bi bi-pencil"></i> Editar Asignación Alumno-Carrera</h3>
                </div>
                ...
                <div class="card-body">
                    ...
                    <form method="post" action="<?php echo base_url('alumno_carrera/update/' . $asignacion['id']); ?>" id="alumnoCarreraForm" novalidate>
                        <?php echo csrf_field(); ?>
                            ...
                            <select class="form-select <?php echo session()->getFlashdata('validation') && session()->getFlashdata('validation')->hasError('id_alumno') ? 'is-invalid' : ''; ?>" 
                                ...
                                <?php foreach ($alumnos as $alumno): ?>
                                    <option value="<?php echo $alumno['id']; ?>" 
                                            <?php echo (old('id_alumno') ?? $asignacion['id_alumno']) == $alumno['id'] ? 'selected' : ''; ?>>
                            ...
                            <select class="form-select <?php echo session()->getFlashdata('validation') && session()->getFlashdata('validation')->hasError('id_carrera') ? 'is-invalid' : ''; ?>" 
                                ...
                                <?php foreach ($carreras as $carrera): ?>
                                    <option value="<?php echo $carrera['id']; ?>" 
                                            <?php echo (old('id_carrera') ?? $asignacion['id_carrera']) == $carrera['id'] ? 'selected' : ''; ?>>
